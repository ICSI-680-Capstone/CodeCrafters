...
import { Trophy, Star, RefreshCw, LayoutDashboard, Construction, Terminal, Users, Handshake } from "lucide-react";
      ...
      {[1, 2, 3].map(i => (
        <Star
          ...
          style={{
            ...
            color: i <= stars ? "#fbbf24" : "rgba(255,255,255,0.12)",
...
function Achievement({ Icon, label, earned }: { Icon: React.ElementType; label: string; earned: boolean }) {
  return (
    <div
    ...
    >
      <Icon
        size={18}
        style={{ color: earned ? "#a78bfa" : "rgba(255,255,255,0.2)", opacity: earned ? 1 : 0.4 }}
      />
      <span style={{ color: earned ? "rgba(255,255,255,0.9)" : "rgba(255,255,255,0.25)" }}>{label}</span>
      {earned && <Star size={11} className="ml-auto text-[#22c55e]" fill="#22c55e" />}
    </div>
  );
}
...
export default function CompletePage() {
  ...
  const rankLabel = stars === 3 ? "Python Legend" : stars === 2 ? "Campus Builder" : "Rising Coder";
  const rankMsg   = stars === 3 ? "Perfect run — you absolutely crushed it!"
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <Trophy size={48} className="mx-auto mb-2 anim-celebrate" style={{ color: "#fbbf24", filter: "drop-shadow(0 0 16px #fbbf2488)" }} />
          <h1
        ...
        <div className="px-8 pb-4 text-center">
          ...
          <div
            className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full text-sm font-black mb-1"
        ...
        <div className="mx-8 mb-5 rounded-2xl overflow-hidden" style={{ height: 220, border: "2px solid rgba(255,255,255,0.1)" }}>
          <CampusCanvas completedStages={5} style={{ width: "100%", height: "100%" }} />
        </div>
        ...
        <div className="mx-8 mb-6">
          ...
          <div className="grid grid-cols-2 gap-2">
            <Achievement Icon={Construction} label="Built 5 buildings" earned={true} />
            <Achievement Icon={Terminal}     label="Python coder"      earned={true} />
            <Achievement Icon={Users}        label="Team player"       earned={true} />
            <Achievement Icon={Handshake}    label="Perfect score"     earned={score >= 450} />
          </div>
        </div>
        ...
        <div className="px-8 pb-8 flex gap-3">
          <button
            ...
            className="flex-1 flex items-center justify-center gap-2 py-3 rounded-2xl font-black text-sm text-white/70 border border-white/15 bg-white/5 hover:bg-white/10 hover:text-white transition-all"
          >
          ...
          <button
            onClick={() => router.push("/dashboard")}
            className="flex-1 flex items-center justify-center gap-2 py-3 rounded-2xl font-black text-sm text-white transition-all hover:-translate-y-0.5"
            style={{
